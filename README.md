...
- The Gradio navigation separates the active **Pipeline** workspace from a dedicated **App settings** page. Language, workspace management, TOML import/export, and server shutdown are kept together on that settings page.
- Every control displays a concise localized explanation of its effect, safe usage, and relevant quality, matching, performance, or data-retention tradeoffs.
...
- The UI uses one global TOML configuration file at `configs/ui/configuration.toml`. **Save configuration**, available on **App settings** and below **Settings by stage**, stores stage selection, workspace root, language, and every pipeline setting together; the file is loaded automatically at application start and **Export settings** downloads it. Use **Import configuration** to load values from another TOML file before saving them globally. Presets are not used in the UI. The global configuration is intentionally ignored by Git because it commonly contains local paths.
- The **App settings** page includes a UI language selector. English and German are supported for navigation, stage names, every setting explanation, page descriptions, status text, and saved configuration values. Language changes apply immediately and are restored on the next launch after saving the configuration.
...
- The General settings include opt-in post-run cleanup checkboxes for source contents, destination metadata, the complete `dst/training` tree, and reference metadata. Cleanup runs only after the full selected pipeline completes successfully. `--remove_training_after_pipeline` preserves `dst/intermediate`; metadata cleanup preserves images and captions.
...
- **Shut down server** on **App settings** requests confirmation before it terminates an active pipeline and closes the local Gradio server completely.
...
- `--remove_src_files_after_pipeline` is a General cleanup option that removes files, `metadata` sidecar folders, and source media subfolders below `src_dir` after all selected stages have completed successfully. Only the `src_dir` root folder is kept; failed or stopped runs do not trigger this cleanup.
...
- `--remove_training_after_pipeline` removes the complete `<dst_dir>/training` folder after a successful full run. It never removes `<dst_dir>/intermediate`, so the detected, classified, and sorted intermediate images remain available for inspection or reuse. The option is disabled by default.
